...
public class FireRules {
	...
	//TODO: NEED TO UPDATE THE WORKING MEMORY SOMEWHERE
	// constructor
	...
	public void fireRules() {
		...
		while (ai.getProgress() < 100 && ai.status.getAttribute(PlayerAttribute.FATIGUE) < 80) {
			...
			for (int i = 0; i < matchedRules.size(); i++) {
				...
				//if the monitored player is working and has progressed more than ai - hack him
				if (w.getIsWorking() == activityValues.Yes && w.getHasProgressedMore() == activityValues.Yes) { 
					ai.easylogic.hackPlayer(player);
				//if the monitored player is working and hasn't progressed more than ai - 
				...
				//if the monitored player is hacking/being hacked and hasn't progressed more than ai -
				//wait for him to finish and then hack him 
					...
					//TODO: make sure the ai keeps working
				//if the monitored player is refreshing and has progressed more - wait for him to go back to his desk and hack him
